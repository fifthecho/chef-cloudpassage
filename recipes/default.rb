...
case node[:platform]
	when "debian", "ubuntu"
		#apt
		command = "echo 'deb http://packages.cloudpassage.com/#{node[:cloudpassage][:repository_key]}/debian debian main' | tee /etc/apt/sources.list.d/cloudpassage.list > /dev/null"
...
case node[:platform]
	when "debian", "ubuntu"
		...
		gpg_key_already_installed = " apt-key list | grep cloudpassage"
	when "redhat", "centos", "fedora"
		command = "rpm --import http://packages.cloudpassage.com/cloudpassage.packages.key"
